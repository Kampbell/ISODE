...
void MapForCpp::generateInclude(ostream& inc, bool debug) const {
	string comment;
	{
		comment = "// Forward declarations.";comment.resize(LL, '-');
		inc << comment << endl;
		inc << "class " << name() << ";" << endl;
	...
	{
		comment = "// Class State."; comment.resize(LL, '-');
		inc << comment << endl;
	...
	{
		comment = "// FSM map default state class.";comment.resize(LL, '-');
		inc << comment << endl;
		...
		comment = "// FSM map states class.";comment.resize(LL, '-');
		inc << comment << endl;
		for (const auto& state : states()) {
			StateForCpp* sfc = dynamic_cast<StateForCpp*>(state.second);
	...
	{
		comment = "// FSM map class.";comment.resize(LL, '-');
		inc << comment << endl;
		inc << "class " << name() << " {" << endl;
		inc << "public:" << endl << tab;
		inc << name() << "();" << endl;
	...
	{
		comment = "// FSM context class ";comment.resize(LL, '-');
		inc << comment << endl;
		...
		inc << "explicit " << endl;
		inc << fsm()->fsmclass() << "() : FSMContext(" << startStateName() << ") {};" << endl;
		inc << fsm()->fsmclass() << "(const statemap::State& state) : FSMContext(state) {};" << endl;
		inc << endl;
...
void MapForCpp::generateCode(ostream& cpp, bool debug) const {
	...
	for (const auto& transition : transitions())
		if (transition.second) {
			...
			context.type() = fsm()->context() + '&';
			TransitionForCpp* tfc = static_cast<TransitionForCpp*>(transition.second);
			cpp << "void " << fsm()->klass() << "State::" << tfc->declaration(&context) << " {" << endl;
