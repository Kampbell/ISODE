...
namespace cpp_ex4 {
	...
	public:
		...
		virtual void Timeout(StoplightContext<Stoplight>& context, LightColors t);
	protected:
	...
	public:
		...
		void Timeout(StoplightContext<Stoplight>& context, LightColors t);
	};
	...
	public:
		explicit 
		StoplightContext() : FSMContext(StopMap::NorthSouthGreen) {};
		StoplightContext(const statemap::State& state) : FSMContext(state) {};
		...
		void Timeout(LightColors t) {
		    setTransition("Timeout");
		    getState().Timeout(*this, t);
		    setTransition(nullptr);
		};
	};
}
